...
fnc_handleFriendlyKill = {  
    _killersCash = killer getVariable "cash";      
    _cashForKill = -50;  
    _lessCash = _killersCash + _cashForKill;  
    if (_killersCash <= 0) then { _cashForKill = 0; };  
    killer setVariable ["cash", _lessCash, true];  
    (uiNameSpace getVariable "myUI_DollarTitle") ctrlSetText format ["Money: $%1",_lessCash];  
};  
